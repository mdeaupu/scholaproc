    ...
    <x-mary-header :title="$isEdit ? 'Ubah Dokumen Legal' : 'Tambah Dokumen Legal Baru'"
        ...
        <x-mary-form wire:submit="save">
                ...
                <div class="lg:col-span-1">
                    <x-mary-card title="Informasi Penting" shadow separator class="border-t-4 border-[#0046FF]">
                        ...
                        <ul class="text-xs text-gray-600 space-y-3 list-disc list-inside leading-relaxed mb-4">
                            <li>Supplier dianggap <span class="font-bold text-green-600">Lengkap & Valid</span> jika
                                minimal
                        ...
                        <div
                            class="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-600 flex gap-2">
            ...
            <div class="flex justify-end gap-3 pt-2">
                @php
                    $cancelRoute = auth()->user()->isSuperAdmin()
                        ? 'superadmin.suppliers.index'
                        : 'admin.suppliers.index';
                @endphp
                <x-mary-button label="Batal" link="{{ route($cancelRoute) }}" class="btn-ghost btn-sm text-black"
                    wire:navigate />
                <x-mary-button label="{{ $isEdit ? 'Simpan Perubahan' : 'Simpan Dokumen' }}" type="submit"
